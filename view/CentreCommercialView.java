...
public class CentreCommercialView extends ProprieteView{

	public CentreCommercialView(CentreCommercial uneCase) {
		super(uneCase);
	...
	public int supplement() {
		...
		for(Case p : MonopolyIUT.getPartie().getPlateau().getListePlateau()) {
			if(p.getView() instanceof CentreCommercialView && ((Propriete)p).estPossedee() && ((CentreCommercialView)p.getView()).possedee && ((Propriete)p).getProprio().equals(((Propriete)uneCase).getProprio())) {
				sup+=1;
			}
		}
		return sup;
	...
	public void setPossedee(boolean b) {
		if(b) {
			prix.setText(((CentreCommercial) uneCase).getLoyer(0)+"€");
			materiels.setText(((CentreCommercial) uneCase).recurence()+"");
		...
		if(modif>=0) {
			if(etat) {
				...
				if(((Propriete)uneCase).estPossedee()&&!possedee) {
					((CentreCommercial)uneCase).vendre();
				}
			}
		}
	}
